...
function sample_proposal_and_compute_log_weights!(
    ...
    @sync for (task_index, particle_indices) in enumerate(index_chunks(1:n_particle; n=filter_data.n_tasks))
...
function sample_proposal_and_compute_log_weights!(
    ...
    # particles 
    @timeit_debug timer "Update states given observations"  update_states_given_observations!(states, observation, model, filter_data, rng)
end
